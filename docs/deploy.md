...
| `WORLD_ENVIRONMENT` | `production`, `staging` or `sandbox`. Which connect URL a proof request points at — a client-side switch on the same app, not a separate one. The docs say only "to test during development, use the [simulator](https://simulator.worldcoin.org) and set `environment` to `staging`" |
