...
| `WORLD_ENVIRONMENT` | `production` or `staging`. A staging app is a separate app in the portal, with an id beginning `app_staging_`, and its QR points at the [simulator](https://simulator.worldcoin.org) rather than at World App. The two must agree — a production app id with `staging` produces a code the app cannot answer. On a mismatched pair the humanity check is switched off and the reason is reported by `/v1/preflight`; the rest of the service is unaffected |
