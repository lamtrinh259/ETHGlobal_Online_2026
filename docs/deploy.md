...
| `VOUCH_REQUIRES_HUMANITY` | Default `true`: a reference is signed and relayed only for a wallet holding a live humanity proof — the platform rule, not a policy; `false` only for a test stack whose writers cannot pass a Selfie Check. |
| `ADMIN_TOKEN` | Demo only, ≥16 chars: enables `/v1/admin/*` (and the `/admin` page) — resetting a Selfie Check on a named account, which forgets its nullifier here and deletes its humanity record on chain (the relayer owns Multipass here, which is what makes that call possible). Answered only where `REGISTRAR_KEY` is set; an enclave-registrar deployment refuses with 501. Unset in production. |
