...
| `POST /v1/admin/privy/delete` | Demo only, `x-admin-token` and `PRIVY_APP_SECRET`: `{wallet}` or `{handle}` → deletes every live record of that wallet on chain (names, answers, references, accounts, humanity), forgets its nullifier, then deletes the Privy user (`DELETE /v1/users/{did}`). For the case Privy refuses to unlink: an account whose only login is the one to reset. The embedded wallet is gone for good. |
| `POST /v1/admin/privy/unlink` | Demo only, `x-admin-token` and `PRIVY_APP_SECRET`: `{wallet}` or `{handle}` → finds the Privy user holding the wallet and unlinks every account but the wallets through `POST /v1/users/{did}/accounts/unlink` on `PRIVY_API_URL` (email, Google, X, GitHub, Discord, LinkedIn…), and deletes the wallet's live platform records on chain (they are keyed by the account, so a fresh wallet attesting the same GitHub would meet `walletMismatch`), so the person can onboard again. Reports what was unlinked, what Privy refused (with its reason; an OAuth account is retried by username when the subject is refused) and what was deleted. |
...
| `GET /v1/graph/:handle` | One person's neighbourhood: them, everyone either side of them, and every edge among that set — which is where "do their referrers refer each other" is answered. `score` is their SybilScore, 0–100: proved humanity is a floor, a writer's one share is split across everyone they vouch for (trust conserved, after SybilRank), weighted by the council's kept reading of each vouch, O(log n) hops; a ring nobody proved sums to nothing. `rank` is trust per connection (SybilRank). |
...
Multipass domain alone (fee 0, registrar `REGISTRAR_ADDRESS`) — the domain existing _is_ the mount, since
