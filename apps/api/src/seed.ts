 ...
 *
 * or, for a deployment that should carry it from the start, `SEED_GRAPH=true` (see `seedOnBoot`).
 *
...
/**
 * Seeding at boot, for a deployment that asks for it.
 *
 * A pull request's preview is a fresh container against a namespace, and a preview with four names and
 * three references has nothing for the reference map to show. `SEED_GRAPH=true` makes the API write the
 * namespace above through its own relay once it is listening — idempotent, so a restart writes nothing
 * twice. It needs the registrar key, since every record is registrar-signed; without one it says so and
 * does nothing. The records are real, on whatever chain the deployment is on.
 */
export async function seedOnBoot(opts: {
  config: {
    SEED_GRAPH: boolean;
    SEED_SALT: string;
    REGISTRAR_KEY?: Hex;
    MULTIPASS_EIP712_NAME: string;
    MULTIPASS_EIP712_VERSION: string;
  };
  apiUrl: string;
  log: (s: string) => void;
  run?: typeof apply;
}): Promise<"skipped" | "seeded" | "failed"> {
  const { config, log, run = apply } = opts;
  if (!config.SEED_GRAPH) return "skipped";
  if (!config.REGISTRAR_KEY) {
    log(
      "seed · SEED_GRAPH is set but REGISTRAR_KEY is not: every seeded record is registrar-signed, so nothing was written"
    );
    return "skipped";
  }
  try {
    ...
    log(`seed · done: ${done.names} names, ${done.humans} proofs, ${done.references} references written`);
    return "seeded";
  } catch (e) {
