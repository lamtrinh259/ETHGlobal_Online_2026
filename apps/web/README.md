...
| `/p/<handle>` | verifier / agent | A person, read in one call (`GET /v1/profile/:handle`). States their records — score, answers, accounts, references received and given — and grades nobody until a reader applies a policy, which the bar on the page does (`?preset=` or `?answers=&minLinks=&minVouches=&humanity=1`). An unheld name says so and offers the two things there are to do with it. A read that failed says that instead, rather than rendering as a person with nothing. |
| `/employers` | verifier | The same reading, asked of everybody under consideration at once: a bar, a shortlist kept in the reader's own browser, and where each of them stands against it. See [docs/employers.md](../../docs/employers.md). |
