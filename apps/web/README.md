...
| `/p/<handle>` | verifier / agent | A person, read in one call (`GET /v1/profile/:handle`). States their records — under the name, the sybil signal in one line (trust from proved humans, who stands behind them and whether they know each other, humanity, how the references read, each linking down to its card) — score, answers, accounts, references received and given, and below them the shape behind the count (who stands behind them, whether those people know each other) and what the references say (each statement read once by the Noolog fast council, summed into one line, every reading behind a fold and marked provisional). Each received reference carries its reading as a badge (supportive / neutral / critical) and a switch sets aside the unsolicited ones, counted; each reference given carries the same badge, and "Request a reference" leads to your own page — and grades nobody until a reader applies a policy, which the bar on the page does (`?preset=` or `?answers=&minLinks=&minVouches=&humanity=1&solicited=1&from=&maxCritical=`; `maxCritical` caps how many references may read as critical by the council, and cannot be met where no council reads). An unheld name says so and offers the two things there are to do with it. A read that failed says that instead, rather than rendering as a person with nothing. |
