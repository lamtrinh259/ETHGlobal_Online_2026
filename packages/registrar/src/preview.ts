 ...
 * `COOLIFY_FQDN` (`1.ketsuban.peeramid.xyz`) or `COOLIFY_URL` (with the scheme). The web app and the
...
export function previewId(fqdn: string | undefined): string | undefined {
  ...
  // A number, and a host beneath it: `1.ketsuban.peeramid.xyz`, never `1` alone.
 ...
 *
 * `https://ketsuban-api.peeramid.xyz` beside `1.ketsuban.peeramid.xyz` is
 * `https://1.ketsuban-api.peeramid.xyz`. Unchanged where there is no preview, where the URL already
